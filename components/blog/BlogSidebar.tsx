...
async function getPopularPosts(currentPostId: number): Promise<BlogPost[]> {
  const [posts, users] = await Promise.all([fetchPosts(), fetchUsers()]);

  const allPosts = transformPostsToBlogPosts(posts, users);

  // Get 3 random posts excluding current post
  const filtered = allPosts.filter((p) => p.id !== currentPostId);
          ...
          {popularPosts.map((post) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-sm font-semibold text-gray-900 mb-1 group-hover:text-brand-purple-600 transition-colors line-clamp-2">
                  {post.title}
                </h4>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 p-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-lg hover:from-purple-700 hover:to-pink-700 transition-colors"
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-wrap gap-2">
          {tags.map((tag) => (
            <Link
              ...
              className="px-3 py-1.5 bg-gray-100 text-gray-700 text-sm rounded-lg hover:bg-brand-purple-100 hover:text-brand-purple-700 transition-colors"
            >
              {tag}
            </Link>
          ))}
